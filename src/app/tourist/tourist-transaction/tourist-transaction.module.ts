import { NgModule } from '@angular/core';
import { CommonModule } from '@angular/common';
import { FormsModule } from '@angular/forms';
import { IonicModule } from '@ionic/angular';

import { TouristTransactionPageRoutingModule } from './tourist-transaction-routing.module';
import { TouristTransactionPage } from './tourist-transaction.page';
import { DateModalModule } from 'src/app/date-modal/date-modal.module';

@NgModule({
  imports: [
    CommonModule,
    FormsModule,
    IonicModule,
    TouristTransactionPageRoutingModule,
    DateModalModule,
  ],
  declarations: [TouristTransactionPage],
})
export class TouristTransactionPageModule {}
